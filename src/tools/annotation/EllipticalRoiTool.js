...
function _createTextBoxContent(
  ...
  if (!isColorImage) {
    ...
    let meanString =
      mean || `Mean: ${numbersWithCommas(mean.toFixed(2))} ${unit}`;
    const stdDevString =
      stdDev || `Std Dev: ${numbersWithCommas(stdDev.toFixed(2))} ${unit}`;
...
function _formatArea(area, hasPixelSpacing) {
  ...
  return area || `Area: ${numbersWithCommas(area.toFixed(2))}${suffix}`;
}
